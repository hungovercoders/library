...
- `~/.claude/hungovercoders/voice/datagriff-facts.md` — the source of truth for every first-person claim. If the topic of the current series isn't covered, you MUST either run `/hc-datagriff-interview <topic>` to capture facts before writing, or write the lessons with impersonal / second-person framing instead of `I wanted…` / `I'll be honest…` openers.
